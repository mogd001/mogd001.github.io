...
exposure <- "A narrow valley in the low level foothills, surrounded by regenerating bush."
...
df <- tibble(datetime = seq(ymd_hms(paste(start_date, "00:00:00", sep = " ")), ymd_hms(paste(as.Date(end_date), format(as.POSIXct(end_date), format = "%H:%M:%S"), sep = " ")), by = "1 hour"))
df$datetime <- as.POSIXct(format(df$datetime), tz = "") # remove timezone to ensure subsequent join is done correctly
...
hrly_plot <- ggplot(hrly_rainfall_df, aes(datetime, rainfall)) +
  ...
  labs(x = "", y = "Rainfall (mm)", title = paste0("Total= ", total_hrly, " mm"))
...
daily_plot <- ggplot(daily_rainfall_df, aes(date, rainfall)) +
  ...
  labs(x = "", y = "Rainfall (mm)", title = paste0("Total= ", total_daily, " mm"))
